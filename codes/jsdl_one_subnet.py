...
def model(x,is_train):
    ...
    with tf.variable_scope("conv2") as scope:
        conv2_s_1 = conv_bn_act(conv1_s_1,[3,3,8,32],activation_func1,is_train,2)
        pool2_s_1 = train.avg_pool(conv2_s_1,5,2,'SAME',3)

    with tf.variable_scope("conv3") as scope:
        conv3_s_1 = conv_bn_act(pool2_s_1,[1,1,32,128],activation_func1,is_train,4)
        pool3_s_1 = train.avg_pool(conv3_s_1,32,32,'SAME',5)
        pool3_reshape = tf.reshape(pool3_s_1, [BATCH_SIZE, 512])
        print "pool3_s_1:{0}".format(pool3_s_1.get_shape())

    with tf.variable_scope('fully_connecting') as scope:
        w1 = train.weight_variable([512,2],6)
        bias = tf.Variable(tf.random_normal([2],mean=0.0,stddev=0.01),name="bias" )
        y_ = tf.nn.softmax(tf.matmul(pool3_reshape, w1) + bias)
#        w1 = train.weight_variable([512,800],6)
#        b1 = tf.Variable(tf.random_normal([800],mean=0.0,stddev=0.01),name="b1" )
#        w2 = train.weight_variable([800,400],7)
#        b2 = tf.Variable(tf.random_normal([400],mean=0.0,stddev=0.01),name="b2" )
#        w3 = train.weight_variable([400,200],8)
#        b3 = tf.Variable(tf.random_normal([200],mean=0.0,stddev=0.01),name="b3" )
#        w4 = train.weight_variable([200,2],9)
#        b4 = tf.Variable(tf.random_normal([2],mean=0.0,stddev=0.01),name="b4" )
#
#        temp = tf.matmul(pool3_s_1, w1) + b1 
